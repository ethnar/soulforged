Vue.component("backend-travel", {
  ...
  created() {
    if (this.actionTarget && this.actionTarget.creatures && this.actionTarget.creatures.some(c => c.hostile || !c.hostile && !c.friendly)) {
  ...
  subscriptions() {
    return {
      tutorialArea: DataService.getIsTutorialAreaStream().do(tutorialArea => {
        if (tutorialArea) {
          this.assault = true;
        }
      })
