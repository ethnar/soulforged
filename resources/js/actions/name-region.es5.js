Vue.component("backend-name-region", {
  ...
  subscriptions() {
    const regionPreferencesPromise = ServerService.request("get-region-name-preferences", {
    ...
    return {
      ...
      mapBox: stream.switchMap(preferences => Rx.Observable.combineLatestMap(preferences.nodeIds.toObject(id => id, id => MapService.getNodeStream(id)))).map(nodes => {
        const box = Object.values(nodes).reduce((acc, node) => ({
